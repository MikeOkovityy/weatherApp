...
class NetworkService {
    ...
    func fetchDailyWeather(lat: Double, lon: Double, completion: @escaping (DailyWeather?, Error?)-> Void) {
        guard let url = URL(string: "https://api.openweatherma(p.org/data/2.5/weather?lat=\(lat)&lon=\(lon)}&appid=\(apiKey)&units=metric") else { return }
        var reguest = URLRequest(url: url)
        reguest.httpMethod = "GET"
